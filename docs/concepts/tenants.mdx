---
title: Tenants
description: Multi-tenant isolation for API keys, agents, environments, vaults, and sessions.
---

AgentStep Gateway groups every API key, agent, environment, vault,
...
| Tenant user | `false` | `X` | Resources in `X` that match the key's scope (`agents`/`environments`/`vaults` allow-lists). |
...
Existing agents/envs/vaults/sessions with `tenant_id = null` are only
visible to global admins. Your seed key is a global admin, so day-to-day
...
- Fallback tuples must also share the primary agent's tenant; mis-
  ...
  reason in the exhaustion error.
