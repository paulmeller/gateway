...
import { Plus, Trash2, Download, Search, ChevronDown } from "lucide-react";
...
export function AgentSkillsTab({ agent }: Props) {
  ...
  const [sourceFilter, _setSourceFilter] = useState<string | null>(null);
  const [results, setResults] = useState<SearchResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [_sources, setSources] = useState<SourceInfo[]>([]);
...
// fetchSkill imported from @/lib/skills
