...
describe("extractKey query-param fallback", () => {
  it("extracts key from x-api-key header (preferred)", () => {
    const req = new Request("http://localhost/v1/files/abc/content", {
  ...
  it("extracts key from ?x-api-key query parameter when no header present", () => {
    const req = new Request("http://localhost/v1/files/abc/content?x-api-key=ck_test789");
    expect(extractKey(req)).toBe("ck_test789");
  });

  it("prefers header over query param", () => {
    const req = new Request("http://localhost/v1/files/abc/content?x-api-key=ck_query", {
  ...
  it("rejects Anthropic passthrough keys in query params (security)", () => {
    const req = new Request(
      "http://localhost/v1/files/abc/content?x-api-key=sk-ant-api03-xxxxxxxxxxxx",
    );
  ...
  it("handles URL-encoded query param values", () => {
    ...
    const req = new Request(
      `http://localhost/v1/files/abc/content?x-api-key=${encodeURIComponent(key)}`,
    );
