...
export const appleProvider = Object.assign(createCliProvider({
  ...
  checkAvailability: async () => {
    ...
    try {
      ...
      // Also check the daemon is running
      const { spawn: spawn2 } = await import("node:child_process");
      const daemonOk = await new Promise<boolean>((resolve) => {
        const proc = spawn2("container", ["system", "status"], {
          stdio: ["pipe", "pipe", "pipe"],
        });
        let out = "";
        proc.stdout?.on("data", (buf: Buffer) => { out += buf.toString(); });
        proc.on("close", (code) => {
          resolve(code === 0 && out.includes("running"));
        });
      ...
      if (!daemonOk) {
        return {
          available: false,
          message: "Apple Containers service not running. Run: brew services start container",
        };
      }
